...
direc = 'C:\Users\W540\Desktop\Semantic Segment\TS7_VarVeins';
matfiles = dir(fullfile(direc, '*.bmp'));
...
for i = 1 : nfiles
   ...
   imgName = string(fullfile(matfiles(i).name));
   [token1,remain1] = strtok(imgName,'_');
   [token2,remain2] = strtok(remain1,'.');
   [token3,remain3] = strtok(token2,'_');
   numchar = strlength(token3);
   imgNum = token3;
   
   img = imread(string(direc)+'\'+string(fullfile(matfiles(i).name)));
   cropImg = img(1:640,1:640);
   
       if numchar == 1
           imwrite(cropImg, 'image_000'+string(token3)+'.bmp')
           % matfiles(i).name = 'image_000'+string(token3)+'.bmp';
       elseif numchar == 2
           imwrite(cropImg, 'image_00'+string(token3)+'.bmp')
           % matfiles(i).name = 'image_00'+string(token3)+'.bmp';
       elseif numchar == 3
           imwrite(cropImg, 'image_0'+string(token3)+'.bmp')
           % matfiles(i).name = 'image_0'+string(token3)+'.bmp';
       elseif numchar == 4
           imwrite(cropImg, 'image_'+string(token3)+'.bmp')
           % matfiles(i).name = 'image_'+string(token3)+'.bmp';
       end
end

   
% %suppose 's' is the struct array. 'DOB' is the field that contains date and time.
% tempSortTable = struct2table(matfiles); % convert the struct array to a table
% sortedTab = sortrows(tempSortTable, 'name'); % sort the table by name
% sortMatfiles = table2struct(sortedTab) % change it back to struct array if necessary


% for i = 1 : nfiles
%     
%     %Find a function in python using the libraries for imread
%    img = imread(string(direc)+'\'+string(fullfile(Matfiles(i).name)));
%    cropImg = img(1:640,1:640);
%    % figure
%    % imshow(cropImg)
%    numchar = strlength(string(fullfile(matfiles(i).name)));
%        if numchar == 11
%            imwrite(cropImg, 'image_000'+string(i)+'.bmp')
%        elseif numchar == 12
%            imwrite(cropImg, 'image_00'+string(i)+'.bmp')
%        elseif numchar == 13
%            imwrite(cropImg, 'image_0'+string(i)+'.bmp')
%        elseif numchar == 14
%            imwrite(cropImg, 'image_'+string(i)+'.bmp')
%        end
%    end


% The code below can go into a function that gets called
% Try to find the other functions in the code below that python does not have already
% once you find all of the functions - you can either find similar functions in python libraries (HIGHLY RECOMMENDED)
% Or you can write all the basic code yourself.
h
%% -------------------------------------------------------------------------------------------------------------------
function [X, map, alpha] = imread(varargin)
%IMREAD Read image from graphics file.
%   A = IMREAD(FILENAME,FMT) reads a grayscale or color image from the file
%   specified by the character vector or string scalar FILENAME. FILENAME
%   must be in the current directory, in a directory on the MATLAB path, or
%   include a full or relative path to a file.
%   
...
%
%   [...] = IMREAD(IRI,...) reads image stored at a remote location. When
...
%   read an image from Amazon S3 cloud specify the full IRI for the image:
